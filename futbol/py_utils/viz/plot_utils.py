...
from py_utils.general import utils


def reorder_timseries_data(x_vals: List[Union[int, float, str]],
                           y_vals: List[Union[int, float]]) -> pd.DataFrame:
    """
...
def plot_descriptive_stats(array: List[Union[int, float]],
                           orient: str,
...
def get_calibrated_plot_fonts(fig_size: tuple) -> Dict[str, float]:
    """
...
def add_plot_skeleton(title: str,
                      x_label: str,
                      y_label: str,
                      fig_size: tuple,
...
def plot_donut(title: str,
    ...
    if not colors:
        colors = utils.get_random_colors(how_many=len(labels))
    _, ax1 = plt.subplots()
...
def plot_bar(title: str,
    ...
    add_plot_skeleton(title=title,
                      ...
                      include_ticks=True)
    if not colors:
        colors = utils.get_random_colors(how_many=len(bar_labels))
    if horizontal:
...
def plot_timeseries(title: str,
    ...
    df_timeseries_data = reorder_timseries_data(x_vals=x_vals, y_vals=y_vals)
    if area_fill:
    ...
    if trim_xticks:
        xticks = utils.linspace_by_index(array=xticks, how_many=5)
    dict_plot_fonts = get_calibrated_plot_fonts(fig_size=fig_size)
    ...
    if symmetrical:
        ...
        if utils.has_negative_number(array=y_vals) and utils.has_positive_number(array=y_vals):
            axis_limit = utils.get_max_of_abs_values(array=y_vals) * 1.05
            plt.ylim(-axis_limit, axis_limit)
    if show_scatter_points:
        plt.scatter(df_timeseries_data['x_vals'],
                    ...
                    label=None)
    if describe:
        plot_descriptive_stats(array=y_vals, orient='h', line_style="-.", line_width=line_width)
...
def plot_histogram(title: str,
    ...
    if describe:
        plot_descriptive_stats(array=array, orient='v', line_style="-.", line_width=line_width)
    if describe: # Showing legend
        dict_plot_fonts = get_calibrated_plot_fonts(fig_size=fig_size)
        legend_label_size = dict_plot_fonts['legend_label_size']
        plt.legend(loc='best', fontsize=legend_label_size)
